...
class TestRoPECPUContract:
    """Contracts that run in ordinary CPU CI instead of being CUDA-skipped."""

    def test_precompute_shape_and_origin(self):
        cos, sin = precompute_rope_cos_sin(17, 64, "cpu", torch.float32)

        assert cos.shape == (17, 32)
        assert sin.shape == (17, 32)
        assert torch.equal(cos[0], torch.ones(32))
        assert torch.equal(sin[0], torch.zeros(32))

    def test_precompute_matches_direct_frequency_formula(self):
        positions, dim, base = 7, 32, 10000.0
        cos, sin = precompute_rope_cos_sin(positions, dim, "cpu", base=base)
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))
        expected = (
            torch.arange(positions, dtype=torch.float32)[:, None] * inv_freq[None, :]
        )

        assert torch.allclose(cos, expected.cos())
        assert torch.allclose(sin, expected.sin())

    def test_standalone_and_fused_rope_tables_match(self):
        fused_cos, fused_sin = precompute_rope_cos_sin(19, 64, "cpu")
        rope_cos, rope_sin = make_half_split_pos_emb(19, 64, "cpu")

        assert torch.equal(fused_cos, rope_cos)
        assert torch.equal(fused_sin, rope_sin)

    @pytest.mark.parametrize(
        "max_seq_len,dim,base",
        [(0, 64, 10000.0), (8, 0, 10000.0), (8, 63, 10000.0), (8, 64, 0.0)],
    )
    def test_precompute_rejects_invalid_configuration(self, max_seq_len, dim, base):
        with pytest.raises(ValueError):
            precompute_rope_cos_sin(max_seq_len, dim, "cpu", base=base)
...
class TestPrefillKernel:
    ...
    def test_prefill_mha_correctness(
        self, device, batch_size, num_heads, seq_len, head_dim
    ):
        ...
        # Run Triton kernel
        out_triton, lse_triton = dominus_ultra_prefill(
            q, k, v, cos, sin, num_kv_heads=num_heads
        )
    ...
    @pytest.mark.parametrize(
        "num_q_heads,num_kv_heads", [(32, 8), (32, 4), (16, 4), (8, 1)]
    )
    def test_prefill_gqa_correctness(self, device, num_q_heads, num_kv_heads):
        ...
        q = (
            torch.randn(
                batch_size, num_q_heads, seq_len, head_dim, device=device, dtype=dtype
            )
            * 0.1
        )
        k = (
            torch.randn(
                batch_size, num_kv_heads, seq_len, head_dim, device=device, dtype=dtype
            )
            * 0.1
        )
        v = (
        ...
        # Run Triton kernel
        out_triton, lse_triton = dominus_ultra_prefill(
            q, k, v, cos, sin, num_kv_heads=num_kv_heads
        )
...
class TestDecodeKernel:
    ...
    @pytest.mark.parametrize("head_dim", [64, 128])
    def test_decode_mha_correctness(
        self, device, batch_size, num_heads, past_len, head_dim
    ):
        ...
        k_cache = (
            torch.randn(
                batch_size, num_heads, past_len, head_dim, device=device, dtype=dtype
            )
            * 0.1
        )
        v_cache = (
        ...
        q_new = (
            torch.randn(batch_size, num_heads, 1, head_dim, device=device, dtype=dtype)
            * 0.1
        )
        ...
        out_triton = dominus_ultra_decode(
            q_new, k_cache, v_cache, cos, sin, num_kv_heads=num_heads
        )

        # Reference: use prefill kernel with full sequence
        # Apply RoPE to cached K
        k_ref = apply_rope_reference(
            k_cache.to(torch.float32), cos[:past_len], sin[:past_len]
        )
        # Apply RoPE to new Q at position past_len
        q_ref = apply_rope_reference(
            q_new.to(torch.float32),
            cos[past_len : past_len + 1],
            sin[past_len : past_len + 1],
        )
    ...
    def test_decode_gqa_correctness(self, device, num_q_heads, num_kv_heads):
        ...
        k_cache = (
            torch.randn(
                batch_size, num_kv_heads, past_len, head_dim, device=device, dtype=dtype
            )
            * 0.1
        )
        v_cache = (
        ...
        q_new = (
            torch.randn(
                batch_size, num_q_heads, 1, head_dim, device=device, dtype=dtype
            )
            * 0.1
        )
        ...
        # Run Triton decode kernel
        out_triton = dominus_ultra_decode(
            q_new, k_cache, v_cache, cos, sin, num_kv_heads=num_kv_heads
        )
        ...
        k_ref = apply_rope_reference(
            k_expanded.to(torch.float32), cos[:past_len], sin[:past_len]
        )
        q_ref = apply_rope_reference(
            q_new.to(torch.float32),
            cos[past_len : past_len + 1],
